...
3. Start the daemon in one terminal:
   ...
   hearth run
   ```
4. In a second terminal, run the veneer client and drive it interactively:
   ```bash
   python -m hearth.veneer.client
   ```
...
4. Repeat step 6 above (a Wikipedia-triggering question) through
   `python -m hearth.veneer.client`. Expect the same shape of response:
...
## 3. Telling environment issues from real bugs
