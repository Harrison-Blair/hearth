...
**Read the "Scout" section of `.fledge/skills/fledge-orchestrate/foraging.md` and follow it exactly** — including the report template at `templates/scout-report.md` in that skill's directory (every section present, in order; `None observed.` where empty).
