...
- **`app.py` is the composition root.** It constructs every concrete implementation, wires routing (keyphrases + intents dict + skills), and injects them into `VoicePipeline`. Construction-time choices live here, not inside components.
- **Routing is two-tier:** `ClassifierRouter` (LLM picks intent label) degrades to `KeyphraseRouter` (cheap substring match → default) on any LLM/JSON failure.
- **Reminders are proactive.** A background `ReminderScheduler` polls SQLite, announces through `AudioArbiter` which serializes against capture to prevent self-triggering.
...
assistant/          # runtime package
  ...
  wake/ stt/ llm/ tts/ nlu/ search/   # each has base.py ABC + concrete impl
  skills/           # Skill subclasses, SkillRegistry
  scheduling/       # ReminderScheduler (proactive)
  storage/          # SQLite ReminderStore
...
- Stub packages and base classes are **deliberate seams** for future phases — don't delete them.
